...
This template and class file "`ascelike-new.cls`" produce manuscripts
that comply with the guidelines of the American Society of Civil
Engineers (ASCE). It has been produced by
[Overleaf](https://www.overleaf.com) in conjunction with the ASCE, and
is based on the unofficial "`ascelike.cls`" developed by Matthew
R. Kuhn.
...
When used together, `ascelike-new.cls` and `ascelike-new.bst` produce
...
-   `\citeNP{key}` produces citations with full author list and year,
    but without enclosing parentheses: e.g. .

-   `\citeA{key}` produces citations with only the full author list:
    e.g.

-   `\citeN{key}` produces citations with the full author list and year,
    but which can be used as nouns in a sentence; no parentheses appear
    around the author names, but only around the year: e.g. states that
    ...

-   `\citeyear{key}` produces the year information only, within
    parentheses, as in [-@Ireland:1954a].

-   `\citeyearNP{key}` produces the year information only, as in .
...
the `ascelike-new.bst` style for the following entries:
