...
function print_word_counts()
	...
	local total_words_out = string.format("%d total %s", total_words, total_words == 1 and "word" or "words")

	local manuscript_words_out = string.format(
		"%d %s in body and notes",
		manuscript_words,
		manuscript_words == 1 and "word" or "words"
	)

	local body_words_out = string.format("%d %s in text body", body_words, body_words == 1 and "word" or "words")

	local note_words_out = note_words > 0
			and string.format(
				"%d %s in notes",
				note_words,
				note_words == 1 and "word" or "words"
			)
		or ""

	local ref_words_out = ref_words > 0
			and string.format(
				"%d %s in reference section",
				ref_words,
				ref_words == 1 and "word" or "words"
			)
		or ""

	local appendix_words_out = appendix_words > 0
			and string.format(
				"%d %s in appendix section",
				appendix_words,
				appendix_words == 1 and "word" or "words"
			)
		or ""
